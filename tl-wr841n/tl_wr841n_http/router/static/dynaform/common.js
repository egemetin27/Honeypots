...
var standardRouterMode = 1;/*±ê×¼ÎÞÏßÂ·ÓÉÄ£Ê½*/
var normal3gMode = 2;/*3GÂ·ÓÉÄ£Ê½*/
var apcMode = 4;/*AP ClientÄ£Ê½*/
var apMode = 8;/*APÄ£Ê½*/
var pure3gMode = 16;/*´¿3GÄ£Ê½*/
...
/*
´Ëº¯Êý´òÓ¡¿ìËÙÉèÖÃÏòµ¼ÖÐµÄÁ÷³ÌÐÅÏ¢¡£Èç£º¿ªÊ¼ >> ÍøÂç½ÓÈë >> WAN¿ÚÁ¬½ÓÀàÐÍ >> 3G >> ÎÞÏß >> Íê³É
²ÎÊýÊ¹ÓÃ£º
isMultiSysMode£ºÊÇ·ñÊÇ¶àÄ£Ê½»úÐÍ
sysMode£ºÄ¿Ç°ÏµÍ³¹¤×÷µÄÄ£Ê½
accessType:3GÄ£Ê½ÏÂÈëÍø·½Ê½£¬Èç3GÓÅÏÈµÈ
isDualBand:ÊÇ·ñÊÇË«Æµ»úÐÍ
labelId£ºÒªÐ´ÈëÁ÷³ÌÐÅÏ¢µÄHTM±êÇ©
stepId£ºÄ¿Ç°Ëù´¦µÚ¼¸²½Á÷³Ì
*/
function showTipMsg(isMultiSysMode, sysMode, accessType, isDualBand, labelId, stepId)
{
	...
	for (i = startIdx; i <= endIdx; i++)
	{
		...
		switch (i)
		{
		case sysModeIdx:/*·Ç¶àÄ£Ê½²úÆ·È¥µôÕâÒ»²½*/
			if (isMultiSysMode != 1)
			{
				nextLoop = 1;
			}
			break;
		case netInIdx:/*·Ç±ê×¼3GÂ·ÓÉÆ÷È¥µôÕâÒ»²½*/
			if (sysMode != normal3gMode)
			{
				nextLoop = 1;
			}
			break;
		case wanOr3gIdx:/*·Ç±ê×¼3GÂ·ÓÉÆ÷»òÕß³¬¹ýÕâÒ»²½µÄÊ±ºòÒ²È¥µôÕâÒ»²½*/
			...
			break;
		case wanTypeIdx:
			if (sysMode != standardRouterMode /*APºÍ´¿3GÂ·ÓÉÆ÷È¥µôÕâÒ»²½*/
				&& sysMode != normal3gMode
				&& sysMode != apcMode)
			{
				nextLoop = 1;
			}
			if (sysMode == normal3gMode && /*3G onlyÈ¥µôÕâÒ»²½*/
			...
			break;
		case net3gIdx:/**/
			if (sysMode != normal3gMode/*APºÍ´¿3GÂ·ÓÉÆ÷È¥µôÕâÒ»²½*/
				&& sysMode != pure3gMode)
			{
				nextLoop = 1;
			}
			...
			break;
		case wlanModeIdx:/*·ÇAPÈ¥µôÕâÒ»²½*/
			if (sysMode != apMode)
			{
				nextLoop = 1;
			}
			break;
		case wlanBandIdx:/*·ÇË«ÆµÂ·ÓÉÆ÷È¥µôÕâÈý²½*/
		case wlan2gIdx:
		case wlan5gIdx:
			if (isDualBand != 1)
			{
			...
			break;
		case wlanIdx:/*Ë«ÆµÂ·ÓÉÆ÷È¥µôÕâÒ»²½*/
			if (isDualBand == 1)
			{
				nextLoop = 1;
			}
			break;
		case wiredIdx:/*·ÇAPÂ·ÓÉÆ÷È¥µôÕâÒ»²½*/
			if (sysMode != apMode)
			{
				nextLoop = 1;
			}
