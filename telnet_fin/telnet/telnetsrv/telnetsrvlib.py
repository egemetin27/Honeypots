...
import csv
from .mal_downloader import maldownloader
...
def command_grab_csv(input_command, csv_file):
    ...
    with open(csv_file, 'r') as csvfile:
        ...
        for row in rows:
            #cropped the b'' in row[0] and row[1] by adding [2:-1]
            if input_command == row[0][2:-3]:
...
def login_prompt_grabber(csv_file):
    '''
    Returns phrases used for asking username and password by the device
    '''
    with open(csv_file, 'r') as csvfile:
        csv_reader = csv.reader(csvfile)
        rows = list(csv_reader)
        username_prompt = rows[3][0]
        password_prompt = rows[3][1]

        #cropping b''
        username_prompt = username_prompt[2:-1]
        password_prompt = password_prompt[2:-1]


        return username_prompt, password_prompt

'''Global Vars by Baris'''
csvfilename = 'deneme_1.csv'
username_prompt, password_prompt = login_prompt_grabber(csvfilename)
'''-------    -   -------'''
...
class TelnetHandlerBase(BaseRequestHandler):
    ...
    PROMPT_USER = username_prompt
    # What prompt to use when requesting a telnet password
    PROMPT_PASS = password_prompt
...
# ------------------------- Telnet Options Engine --------------------------
    ...
    def options_handler(self, sock, cmd, opt):
        ...
        if cmd == NOP:
            ...
            log.debug("Unhandled option: TODO")
...
# ------------------------------- Input Cooker -----------------------------
    def _inputcooker_getc(self, block=True):
        ...
        try:
            ...
            print("error _inputcooker_getc :", e)
    ...
    def inputcooker_socket_ready(self):
        ...
        print("NOT IMPLEMENTED inputcooker_socket_ready")
        return
    ...
    def inputcooker_store_queue(self, char):
        ...
        print("NOT IMPLEMENTED inputcooker_store_queue")
        return
...
# ----------------------- Command Line Processor Engine --------------------
    ...
    def authentication_ok(self):
        try:
            print("*****************Login Attempt, checking for auth**************")
            ...
            if self.authCallback:
                while attempt < max_attempt:
                    ...
                    try:
                        ...
                        self.username = None
                        attempt += 1 # baris edit
                        self.writeline(error_message) # sending the error message
            ...
            print("An error occured in authentication_ok")            
    ...
    def handle(self):
        try: 
            ...
            file1.write("Logs of the current user:\n")
            file1.close()
            self.session_start()
            while self.RUNSHELL:
                raw_input = self.readline(prompt=self.PROMPT).strip()
                file1 = open("logs.txt", "a")
                file1.write(raw_input + "\n")
                file1.close()
                ...
                if self.input.cmd:
                    ...
                    if self.COMMANDS.has_key(cmd):
                        ...
                        out_command = command_grab_csv(self.raw_input, csv_file)
                        print("-out command:", out_command)
                        print("-input(type):", self.input)
                        print("-input raw:", self.input.raw)
                        print("-params:", self.input.params)

                        if 'wget' in self.raw_input:
                            maldownloader(self.raw_input)
                            print("if true - wget is entered")
                        else:
                            print("else - wget is NOT entered")
                            #no such command in csv
